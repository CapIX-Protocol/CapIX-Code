...
describe("Plugin — Route interception (auto mode)", () => {
  ...
  it("should pass through when model is not auto (bare)", async () => {
    const msg = makeMessage("Write a function");
    const result = await capixSmartRoute.onMessage!(msg as any, makeContext("openai/gpt-4o") as any);
    expect(result).toBe(msg);
  });
...
describe("Plugin — Error propagation", () => {
