...
function mockCatalogResponse(models: Array<{ model: string; provider: string; spotPrice: number; status: string }>) {
...
describe("SmartRouter — Construction & Memory", () => {
  ...
  it("should load memory from disk when file exists", () => {
    ...
    (fs.readFileSync as ReturnType<typeof vi.fn>).mockReturnValue(
      JSON.stringify({
        ratings: { "model-a": { reasoning: { score: 0, selections: 3, overrides: 1 }, coding: { score: 0, selections: 0, overrides: 0 } } },
...
describe("SmartRouter — pickBestModel (catalog scoring)", () => {
  ...
  it("should penalize expensive models", () => {
    const models = [
      makeModel("cheap-model", 0.001),
      makeModel("expensive-model", 0.1),
    ];
    const best = router.pickBestModel(models, "reasoning");
  ...
  it("should prefer cheaper model when scores are tied", () => {
    const models = [
      makeModel("model-x", 0.005),
      makeModel("model-y", 0.001),
    ];
    const best = router.pickBestModel(models, "reasoning");
...
describe("SmartRouter — Learning (memory updates)", () => {
  ...
  it("should record override multiple times correctly", () => {
...
describe("SmartRouter — routeAuto (classification + catalog)", () => {
  ...
  it("should classify as reasoning and route to reasoning model", async () => {
    ...
    const result = await router.routeAuto("Explain how quicksort works", "session-1", "https://capix.network/api/v1", "test-key");
  ...
  it("should classify as coding and route to coding model", async () => {
    ...
    const result = await router.routeAuto("Write a sort function", "session-2", "https://capix.network/api/v1", "test-key");
  ...
  it("should default to coding on classify failure", async () => {
    ...
    const result = await router.routeAuto("Explain something", "session-3", "https://capix.network/api/v1", "test-key");
  ...
  it("should fall back to cached models on catalog fetch failure", async () => {
    const catalog = [
      { model: "qwen2.5-coder-7b", provider: "capix", spotPrice: 0.001, status: "live" },
    ];
    ...
    const result = await router.routeAuto("Write more code", "session-5", "https://capix.network/api/v1", "test-key");
  ...
  it("should use fallback model when catalog is empty and fetch fails", async () => {
    ...
    const result = await router.routeAuto("Write code", "session-6", "https://capix.network/api/v1", "test-key");
  ...
  it("should cache classification per session", async () => {
    ...
    const fetchMock = vi.fn().mockImplementation((url: string) => {
      ...
      if (url.includes("chat/completions")) return Promise.resolve(mockClassifyResponse("reasoning"));
      return Promise.resolve(mockFetchResponse({}));
    });
    globalThis.fetch = fetchMock as unknown as typeof fetch;

    // First call — should classify
    await router.routeAuto("Explain architecture", "session-7", "https://capix.network/api/v1", "test-key");
...
describe("SmartRouter — Structured logging (W5-T1)", () => {
  ...
  it("should log classification with task type and prompt", async () => {
    ...
    await router.routeAuto("Explain distributed systems", "session-log3", "https://capix.network/api/v1", "test-key");
