...
describe("session.message-v2.fromError", () => {
  ...
  test("serializes CapixHttpError with supportId and capixCode into APIError metadata", () => {
    const error = Object.assign(new Error("Route temporarily unavailable"), {
      name: "CapixHttpError",
      status: 503,
  ...
  test("serializes CapixHttpError without optional fields as message-only metadata", () => {
    const error = Object.assign(new Error("request aborted by caller"), { name: "CapixHttpError" })
