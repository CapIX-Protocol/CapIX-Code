...
describe("SessionNs.getUsage", () => {
  ...
  test("uses authoritative Capix receipt cost when provided", () => {
    // The capix/auto catalog list price is zero — the receipt's provisional
    // cost (forwarded as capix.costUsd provider metadata) is the only real
    // billed amount.
    const result = SessionNs.getUsage({
      model: createModel({
        context: 100_000,
      ...
      usage: usage({ inputTokens: 11_774, outputTokens: 39, totalTokens: 11_813 }),
      metadata: { capix: { receiptId: "receipt-1", costUsd: 0.0042 } },
    })

    expect(result.tokens.input).toBe(11_774)
    expect(result.tokens.output).toBe(39)
    expect(result.cost).toBe(0.0042)
  })

  test("falls back to list-price cost when the Capix receipt cost is invalid", () => {
    const result = SessionNs.getUsage({
      model: createModel({
        context: 100_000,
        output: 32_000,
      ...
      usage: usage({ inputTokens: 1_000_000, outputTokens: 100_000, totalTokens: 1_100_000 }),
      metadata: { capix: { costUsd: "not-a-number" } },
    })
