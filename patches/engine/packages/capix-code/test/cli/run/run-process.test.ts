...
describe("opencode run (non-interactive subprocess)", () => {
  ...
  // A terminal provider error (e.g. 402, or a non-transient 5xx) fails the
  // turn and publishes session.error. The headless loop must abort there and
  // the process must exit nonzero promptly — it used to print the error and
  // then wait forever for an idle event, so VM-side watchdogs had to kill it.
  cliIt.concurrent(
    "exits nonzero promptly on a terminal provider error",
    ({ llm, opencode }) =>
      Effect.gen(function* () {
        yield* llm.error(402, {
          type: "error",
          error: { code: "insufficient_balance", message: "Payment required" },
        })
        const result = yield* opencode.run("charge me", { timeoutMs: 15_000 })
        expect(result.exitCode).not.toBe(0)
        expect(result.durationMs).toBeLessThan(15_000)
      }),
    30_000,
  ...
  cliIt.concurrent(
    "--format json emits an error record and exits nonzero on a terminal provider error",
    ({ llm, opencode }) =>
      Effect.gen(function* () {
        yield* llm.error(402, {
          type: "error",
        ...
        const result = yield* opencode.run("charge me", { format: "json", timeoutMs: 15_000 })
        expect(result.exitCode).not.toBe(0)
        expect(result.durationMs).toBeLessThan(15_000)
