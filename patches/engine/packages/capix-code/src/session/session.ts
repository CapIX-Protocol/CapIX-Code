...
export const getUsage = (input: { model: Provider.Model; usage: Usage; metadata?: ProviderMetadata }) => {
  ...
  // Capix receipts carry the authoritative billed cost (USD, settled from the
  // gateway's provisional cost) — the catalog list price for capix/auto is
  // zero, so prefer the receipt like the Copilot path prefers totalNanoAiu.
  const capixCostUsd = input.metadata?.["capix"]?.["costUsd"]
  return {
    cost:
      typeof capixCostUsd === "number" && Number.isFinite(capixCostUsd) && capixCostUsd >= 0
        ? capixCostUsd
