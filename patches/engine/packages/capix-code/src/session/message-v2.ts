...
export function fromError(
  ...
  switch (true) {
    ...
    case e instanceof Error && e.name === "CapixHttpError": {
      // Capix gateway error (src/capix-provider.ts in the customer runtime).
      // Preserve the supportId/traceId and capixCode so a customer can quote
      // them to support; the human-readable message stays byte-identical.
      // Duck-typed on `name` because the provider class lives outside this
      // compilation unit. isRetryable is always false and statusCode is
      // omitted on purpose: the engine's retry policy retries any retryable
      // (or 5xx) APIError without an attempt cap, while Capix route retries
      // are already bounded inside the model invoker.
      const capix = e as Error & { status?: number; capixCode?: string; supportId?: string }
      return new APIError(
        {
          message: capix.message,
          isRetryable: false,
          metadata: {
            ...(capix.capixCode ? { code: capix.capixCode } : {}),
            ...(capix.supportId ? { supportId: capix.supportId } : {}),
            ...(typeof capix.status === "number" && capix.status > 0 ? { status: String(capix.status) } : {}),
          },
        },
        { cause: e },
      ).toObject()
