...
// CLI entry point for `opencode run` and `capix-code --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`capix-code --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`capix-code --mini --attach`): connects to a running
//      Capix Code server and runs interactive mode against it.
//
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run Capix Code with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running Capix Code server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to CAPIX_CODE_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to CAPIX_CODE_SERVER_USERNAME or 'capix')",
      })
      .option("dir", {
        type: "string",
    ...
    yield* Effect.promise(async () => {
      ...
      async function execute(sdk: OpencodeClient) {
        ...
        async function loop(client: OpencodeClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
          ...
          for await (const event of events.stream) {
            ...
            if (event.type === "session.error") {
              ...
              // A session error is terminal for a headless run: the turn has
              // already failed, so stop consuming events instead of waiting
              // for an idle status that may never arrive.
              break
            }
        ...
        if (!interactive) {
          ...
          // A headless run must terminate once the result is printed: plugin
          // side effects (fs watchers, supervised MCP servers) and the open
          // event subscription can otherwise keep the event loop alive
          // forever. Flush stdio first so JSONL consumers see every event.
