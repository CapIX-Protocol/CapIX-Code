...
export class McpSupervisor {
  ...
  private health: McpHealth = { state: 'disconnected', toolCount: 0, lastCheckedAt: '', restartCount: 0 };
  private timer: NodeJS.Timeout | null = null;
  private listeners: Set<(h: McpHealth) => void> = new Set();

  start(mcpPath: string, env: Record<string, string>): void {
    this.health = { state: 'starting', toolCount: 0, lastCheckedAt: new Date().toISOString(), restartCount: 0 };
  ...
  private spawnProcess(mcpPath: string, env: Record<string, string>): void {
    ...
    try {
      this.process = spawn('node', [mcpPath, 'server', '--stdio'], {
        env: { ...process.env, ...env },
        stdio: ['pipe', 'pipe', 'pipe'],
      });

      this.process.on('exit', (code) => {
        if (code !== 0 && this.health.restartCount < MAX_RESTARTS) {
          this.health.state = 'degraded';
          this.health.error = `Process exited with code ${code}`;
          this.notify();

          const backoff = RESTART_BACKOFF_MS[Math.min(this.health.restartCount, RESTART_BACKOFF_MS.length - 1)];
          this.health.restartCount++;
          this.notify();

          setTimeout(() => this.spawnProcess(mcpPath, env), backoff);
        } else if (code !== 0) {
    ...
    this.process.stdout?.once('data', (data: Buffer) => {
      try {
        ...
        if (response.result?.tools) {
          ...
          this.health.state = this.health.toolCount >= EXPECTED_MIN_TOOLS ? 'connected' : 'degraded';
